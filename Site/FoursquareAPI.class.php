...
class FoursquareApi {
	...
	/** @var String $Mode Swarm and Foursquare */
	private $Mode;
	 ...
	 * @param String $mode Defaults to foursquare
     */
	public function  __construct($client_id = false,$client_secret = false, $redirect_uri='', $version='v2', $language='en', $api_version=DEFAULT_VERSION, $mode = DEFAULT_MODE){
	...
	/**
	 * GeoLocate_names - Editado por Daniel Radd ***
	 * Aproveita a api google maps para gerar um par de lat / lng para um determinado endereço 
	 * Embalado com FoursquareApi para facilitar as pesquisas de localidade.
	 * @param String $addr An address string accepted by the google maps api
	 * @return array(lat, lng) || NULL
	 */
	public function GeoLocate_names($addr){
		$geoapi = "http://maps.googleapis.com/maps/api/geocode/json";
		$params = array("address"=>$addr,"sensor"=>"false");
		$response = $this->GET($geoapi,$params);
		$json = json_decode($response);
		...
		// 1º cidade, 2º estado, 3º pais
			return array($json->results[0]->address_components[1]->long_name,
